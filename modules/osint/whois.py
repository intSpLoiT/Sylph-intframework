...
def format_ip(ip): #Source: https://github.com/secynic/ipwhois/blob/master/ipwhois/utils.py
...
def rdap_query(query, output):
        for i in query:
                try:
                        ...
                        rdap = r.json()
                        name = rdap["name"].title() #id assigned in network registration for given IP address
                        ...
                        print("{}Error retreiving Whois for '{}'. Ensure you have provided a valid IP address{}".format(colors.red, i, colors.normal))
                        continue
...
def main():
        parser = argparse.ArgumentParser(description='Whois - A python based Whois retriever/parser')
        ...
        querygroup.add_argument('-f', '--file', metavar='file', nargs='?', help="A new line delimited file containing IP addresses or domain names")
        querygroup.add_argument('-r', '--reverse', action='store_true', help="Retrieve IP address for given domain(s)")
        outgroup = parser.add_mutually_exclusive_group()
        outgroup.add_argument('-v', '--version', help='show program\'s version number and exit', action='version', version='%(prog)s 1.0.0')
